...
public class TimetableGenerationService {
        ...
        for (SubjectAllocation allocation : allocations) {
            ...
            if (subject.getLabHrsPerWeek() != null && subject.getLabHrsPerWeek() > 0) {
                ...
                for (int i = 0; i < labSessions; i++) {
                    ...
                    if (placed) totalPlaced += 2;
                    else conflicts.add(new ConflictDto(subject.getSubjectName(), section.getName(),
    ...
    private Resource pickRoom(Subject subject, Section section) {
        String roomType = (subject.getType() != null && subject.getType().equalsIgnoreCase("LAB")) ? "LAB" : "CLASSROOM";
        if (section.getStudentStrength() == null) return null;
        ...
        for (String day : DAYS) {
            for (int slot = 1; slot < TOTAL_SLOTS; slot++) {
                ...
                if (isFacultyFree(faculty.getId(), day, slot) && isFacultyFree(faculty.getId(), day, next)
                    ...
                    e1.setSection(section); e1.setSubject(subject); e1.setFaculty(faculty);
                    e1.setRoom(room); e1.setSemester(semester); e1.setSession(session);
                    e1.setDayOfWeek(day); e1.setSlotNumber(slot); e1.setLocked(false);
                    entryRepo.save(e1);

                    TimetableEntry e2 = new TimetableEntry();
                    e2.setSection(section); e2.setSubject(subject); e2.setFaculty(faculty);
                    e2.setRoom(room); e2.setSemester(semester); e2.setSession(session);
                    e2.setDayOfWeek(day); e2.setSlotNumber(next); e2.setLocked(false);
                    entryRepo.save(e2);
