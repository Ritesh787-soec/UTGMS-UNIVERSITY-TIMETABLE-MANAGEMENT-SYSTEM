...
public interface TimetableEntryRepository extends JpaRepository<TimetableEntry, Long> {
    ...
    boolean existsByFacultyIdAndDayOfWeekAndSlotNumberAndIdNot(Long facultyId, String dayOfWeek, Integer slotNumber,
            Long id);

    boolean existsByRoomIdAndDayOfWeekAndSlotNumberAndIdNot(Long roomId, String dayOfWeek, Integer slotNumber, Long id);

    boolean existsBySectionIdAndDayOfWeekAndSlotNumberAndIdNot(Long sectionId, String dayOfWeek, Integer slotNumber,
            Long id);
    ...
    int countByFacultyIdAndSemesterIdAndSession(@Param("facultyId") Long facultyId,
            @Param("semesterId") Long semesterId, @Param("session") String session);

    @Query("SELECT COUNT(e) FROM TimetableEntry e WHERE e.room.id = :roomId AND e.semester.id = :semesterId AND e.session = :session")
    int countByRoomIdAndSemesterIdAndSession(@Param("roomId") Long roomId, @Param("semesterId") Long semesterId,
            @Param("session") String session);

    @Query("SELECT e FROM TimetableEntry e WHERE e.semester.id = :semesterId AND e.session = :session")
    List<TimetableEntry> findBySemesterIdAndSession(@Param("semesterId") Long semesterId,
            @Param("session") String session);
